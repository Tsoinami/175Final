...
def negCropping():
    ...
    for name in names:
        ...
        for row in xrange(0,endCol-170,170):
            # i think here we need to add a check to see whether or not we have
            # fallen out of bound in row
            if row + 170 > endRow:
                break
            for col in xrange(0,endRow-170,170):
                ...
                # cv2.imwrite("neg_processed_data/" + str(name)+".jpg", crop_img)
